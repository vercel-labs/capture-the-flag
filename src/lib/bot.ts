...
function registerHandlers(instance: Chat) {
  instance.onSlashCommand("/ctf", async (event) => {
...
async function handleQuickStart(event: SlashCommandEvent) {
  await event.channel.post(
...
async function handleStatus(event: SlashCommandEvent) {
...
async function handleLeaderboard(event: SlashCommandEvent) {
...
async function handleStop(event: SlashCommandEvent, matchId: string) {
  const activeMatchIds = await redis.smembers(redisKeys.activeMatches);
